...
void Reversi::reDrawGameCell(const Cell& attack, char newPlacer) {
  for (unsigned int r = 0; r < gameCells.size(); ++r) {
    for (unsigned int c = 0; c < gameCells[r].size(); ++c) {
      if (gameCells[r][c].compareCell(attack)){
        gameCells[r][c].setPlacer(newPlacer);
        return; // will not do extra controlling
      }
    }
  }
}
...
// this function play one step for computer
// if there is not available step will return false
bool Reversi::play() {
  ...
  cout << "Best Attack = row: '" << attack.getX() << "' - column '" <<
          attack.getY() << "' - who:" << attack.getPlacer() << endl;
...
// this function will control all direction and will count how many enemy can
// eat according to whoPlay then will return max cruel cell
// if there is no available will return cell which out of board (-1,-1))
...
// yol ustundeki dusmanlari sayar
// playeri vectorde arar taki playerin zitti yani kendini bulana kadar
// eger bu fonksiyon oyuncu icin yol ariyorsa player x gibi davranir
// kac tane x varsa onu bulur sonra kendine bulunca durur
...
bool Reversi::isInBoard(const Cell& item)const {
  for (unsigned int r = 0; r < gameCells.size(); ++r) {
    for (unsigned int c = 0; c < gameCells[r].size(); ++c) {
      if (gameCells[r][c].compareCell(item))
        return true;
    }
  }
  return false;
...
// it is a control function for programmer 
...
// if all step is okey, this function will add a cell on gamecell vector
// finds exact place end insert there a cell or a vector of cell
